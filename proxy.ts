import {roleFromDatabase} from "./lib/permissions";
...
export const proxy=auth(async request=>{
 ...
 const session={id:user.id,role:roleFromDatabase(user.role)!,advertiserIds,advertiserId:advertiserIds[0]??"",source:"session" as const};
 ...
 for(const advertiserId of [...query,...(detail?[detail]:[])]){
  const row=await db.advertiser.findFirst({where:{id:advertiserId,status:"ACTIVE",...(user.role==="SUPER_ADMIN"?{}:{members:{some:{userId:user.id}}})},select:{id:true}});if(!row)denied=true;
 }
