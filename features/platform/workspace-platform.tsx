"use client";
import {activityLabel} from "@/lib/product-language";
...
function WorkspaceProfile({id,compact}:{id:string;compact:boolean}){
 ...
 {admin&&<div className="platform-actions"><Link href={`/settings?advertiser=${id}`}>담당자 배정 · 사용자 관리</Link><Link href={`/advertisers?advertiser=${id}`}>기본 정보 관리</Link><Link href={url}>데모 연결 설정</Link><button disabled={model.busy||!steps.slice(0,4).every(Boolean)} onClick={async()=>{if(await model.save({action:'start'}))window.location.assign(`/dashboard?advertiser=${id}`);}}>분석 시작</button></div>}
 {view.document.started&&<div className="platform-tour"><b>다음 세 가지를 확인하세요.</b><Link href={`/dashboard?advertiser=${id}`}>1. 데이터를 확인하세요</Link><Link href={`/funnel?advertiser=${id}`}>2. 고객 이탈을 찾으세요</Link><Link href={`/campaigns?advertiser=${id}`}>3. 실행안을 작성하세요</Link></div>}
 <details><summary>최근 활동</summary><ol>{(view.activity??view.document.activity).map(a=><li key={a.id}><time>{new Date(a.at).toLocaleString('ko-KR')}</time> · {activityLabel(a.label)}</li>)}</ol>{!(view.activity??view.document.activity).length&&<p>아직 기록된 설정 활동이 없습니다.</p>}</details></>}
 </section>;
}
