...
function WorkspaceProfile({id,compact}:{id:string;compact:boolean}){
 ...
 {admin&&<div className="platform-actions"><Link href={`/settings?advertiser=${id}`}>담당자 배정 · 사용자 관리</Link><Link href={`/advertisers?advertiser=${id}`}>기본 정보 관리</Link><Link href={url}>데모 연결 설정</Link><button disabled={model.busy||!steps.slice(0,4).every(Boolean)} onClick={async()=>{if(await model.save({action:'start'}))window.location.assign(`/?advertiser=${id}`);}}>운영 시작</button></div>}
 {view.document.started&&<div className="platform-tour"><b>다음 세 가지를 확인하세요.</b><Link href={`/?advertiser=${id}`}>1. 데이터를 확인하세요</Link><Link href={`/funnel?advertiser=${id}`}>2. 고객 이탈을 찾으세요</Link><Link href={`/campaigns?advertiser=${id}`}>3. 개선 캠페인을 만드세요</Link></div>}
