...
import {loadDecisionWorkspace,setSignalStatus,signalPreview,signalHandoff,signalJourneyUrl,type DecisionWorkspace} from "@/lib/decision/service";
...
export function DecisionFeed({advertiserIds,period,all=false,readOnly=false,title="오늘의 운영 브리핑",perAdvertiser=false}:{advertiserIds:string[];period:Period;all?:boolean;readOnly?:boolean;title?:string;perAdvertiser?:boolean}){
  ...
  return <section className="decision-feed" aria-label={title} data-testid="decision-feed"><header><div><span className="decision-eyebrow">DATA → SIGNAL → ACTION</span><h2>{title}</h2><p>선택한 {period}일과 동일 길이의 직전 기간 · 명확한 규칙으로 감지한 변화</p></div>{all&&<label><input type="checkbox" checked={hidden} onChange={e=>setHidden(e.target.checked)}/> 숨긴 신호 포함</label>}</header>
    {error?<p role="alert">{error} <button onClick={()=>setAttempt(v=>v+1)}>다시 불러오기</button></p>:!ready?<p role="status">운영 신호를 확인하고 있습니다…</p>:<>
    {!signals.length&&<p className="decision-empty">현재 확인이 필요한 큰 변화가 없습니다. 숨긴 신호는 Operations에서 다시 확인할 수 있습니다.</p>}
    <ol className="decision-list">{signals.map(s=><li key={s.id} className="decision-row" data-signal-type={s.type} data-signal-id={s.id}><div className="decision-meta"><span>{s.advertiserName}</span><span data-severity={s.severity}>{severityNames[s.severity]}</span><span>{signalNames[s.type]} · {s.source}</span></div><h3>{s.title}</h3><p>{s.summary}</p><div className="decision-impact"><strong>{s.evidence[0]?.label} · {value(s.evidence[0]?.current??null,s.evidence[0]?.unit??"")}</strong>{s.evidence[0]?.change!==null&&<span>직전 대비 {value(s.evidence[0]?.change??null,s.evidence[0]?.changeUnit??"")}</span>}<span>영향 고객 {number(s.affectedUsers)}명</span>{s.audience&&<span>재공략 가능 <b>{number(s.audience.size)}명</b> · {s.audience.window}일</span>}</div>
      ...
      <div className="decision-next"><p><b>다음 행동</b> {s.recommendedAction}</p><p>현재 대응 · {statusNames[status(s)]}{!!workspaces.find(w=>w.advertiserId===s.advertiserId)?.draftCounts?.[s.id]&&` · 연결된 초안 ${workspaces.find(w=>w.advertiserId===s.advertiserId)!.draftCounts![s.id]}건`}</p><div className="decision-actions">{s.type==="DATA_ISSUE"?(can(user.role,"VIEW_CONNECTIONS")&&<Link href={`/connections?advertiser=${encodeURIComponent(s.advertiserId)}`}>데이터 연결 확인</Link>):s.stage&&<Link href={signalJourneyUrl(s)}>고객 흐름 보기</Link>}{!s.stage&&s.type!=="DATA_ISSUE"&&<Link href={`/${s.type==="CAMPAIGN_FATIGUE"?"campaigns":"performance"}?advertiser=${encodeURIComponent(s.advertiserId)}&period=${s.period}`}>{s.type==="CAMPAIGN_FATIGUE"?"캠페인 확인":"성과 확인"}</Link>}{canAct&&s.audience&&<button onClick={()=>setSelected(s)}>재공략 설계</button>}{canReview&&all&&<label>검토 상태<select aria-label={`${s.title} 검토 상태`} value={status(s)} disabled={busy} onChange={e=>change(s,e.target.value as SignalStatus)}>{(Object.keys(statusNames) as SignalStatus[]).map(k=><option key={k} value={k}>{statusNames[k]}</option>)}</select></label>}</div></div></li>)}</ol>
...
function AudienceDialog({signal,workspace,close}:{signal:Signal;workspace:DecisionWorkspace;close:()=>void}){
  ...
  return <dialog ref={ref} className="decision-dialog" aria-labelledby="decision-audience-title" onCancel={close}><button className="decision-close" onClick={close}>닫기</button><span className="decision-eyebrow">SIGNAL → AUDIENCE → CAMPAIGN</span><h2 id="decision-audience-title">재공략 대상 확인</h2><p>{signal.title}</p><strong className="decision-size">{number(a.size)}명</strong><h3>{a.name}</h3><dl><dt>Include</dt><dd>{a.sourceNames.join(" OR ")}<br/>{a.event} · 최근 {a.window}일</dd><dt>Exclude</dt><dd>구매 완료 고객 · 더 깊은 행동 그룹</dd><dt>데이터 출처</dt><dd>{signal.sourceDescription}</dd><dt>추천 목표</dt><dd>{preview?.draft.objective??"데이터 확인 필요"}</dd></dl><p>선택 기간 {signal.period}일 내 {a.window}일 행동 기준입니다. Journey의 동일 세그먼트와 Campaign의 모집단을 사용합니다.</p>
    ...
    <p>다음 화면에서 채널·메시지·예산을 선택하세요. Meta는 DEMO, Naver·Google은 Coming Soon입니다.</p>{(problem||error)&&<p role="alert">{problem||error}</p>}<button className="decision-primary" disabled={!preview} onClick={()=>{try{router.push(saveCampaignHandoff(signalHandoff(signal,context!)))}catch{setError("캠페인 연결에 실패했습니다. 고객 그룹을 다시 확인해 주세요.")}}}>캠페인 설계</button>
  </dialog>;
}
