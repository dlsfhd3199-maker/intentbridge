...
import { ArrowLeft, ArrowRight, FlaskConical, RotateCcw, Sparkles } from "lucide-react";
...
function LabContent({ baseline: b }: { baseline: BaselineMetrics }) {
  ...
  return <div className="pl">
    ...
    <section className="pl-baseline" aria-label="핵심 성과 지표">{baselineCards.map(card => <div key={card.label}><span>{card.label}</span><strong>{card.value}</strong></div>)}</section>
    ...
    <Link href="/" className="pl-back"><ArrowLeft size={16}/> 홈 대시보드로 돌아가기</Link>
  </div>;
}
