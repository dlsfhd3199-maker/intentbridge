...
function WorkspaceContent({ data }: { data: FunnelWorkspaceData }) {
  ...
  return <div className="fw">
    <section className="fw-route" aria-label="캠페인 개요"><div className="fw-route-title"><span className="fw-route-icon"><Network size={23}/></span><div><h2>{data.campaign.name}</h2><p>{data.advertiser.name} · 최근 {data.query.period}일 · {supported ? "GPT Organic + ChatGPT Ads → Meta Retargeting" : `${sourceName} → ${retargetName}`}</p></div><span className="fw-badge">{supported ? data.campaign.status : "MOCK 준비 예정"}</span></div>
      <div className="fw-channel-controls"><span>Source Channel</span><label><span className="sr-only">Source Channel</span><select aria-label="Source Channel" value={sourceChannel} onChange={event => setSourceChannel(event.target.value)}>{data.channels.map(channel => <option key={channel.id} value={channel.id}>{channel.name}</option>)}</select></label><ArrowRight size={17}/><span className="fw-site-step">Site Behavior</span><ArrowRight size={17}/><label><span className="sr-only">재방문 광고 Channel</span><select aria-label="Retargeting Channel" value={retargetChannel} onChange={event => setRetargetChannel(event.target.value)}>{data.channels.map(channel => <option key={channel.id} value={channel.id}>{channel.name}</option>)}</select></label><span>재방문 광고 Channel</span></div>
      <p className="fw-route-note"><b>GPT 전용 구조가 아닙니다.</b> 유입 매체와 재공략 매체를 확장할 수 있습니다. 현재 데이터는 ChatGPT → Meta 조합만 제공합니다.</p>
    </section>
