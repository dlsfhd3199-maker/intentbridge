...
export function OperationsSummary({advertiserId,period}:{advertiserId:string;period:Period}) {
  ...
  return <section className="operations-home"><div>{["검토 완료 실행안","확인 필요","운영 제안"].map((label,i)=><span key={label}>{label}<b>{counts[i]}</b></span>)}</div><Link href={`/operations?advertiser=${advertiserId}&period=${period}`}>운영 제안 열기 →</Link></section>;
}
