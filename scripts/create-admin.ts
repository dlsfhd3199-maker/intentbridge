...
async function main(){
 ...
 await db.$transaction(async tx=>{
  const existing=await tx.user.findUnique({where:{email}}),admins=await tx.user.count({where:{role:"SUPER_ADMIN",status:"ACTIVE"}});
  if(existing){if(existing.role!=="SUPER_ADMIN"||existing.status!=="ACTIVE"||existing.passwordHash||admins!==1)throw new Error("Admin bootstrap refused. Use existing account management.");await tx.user.update({where:{id:existing.id},data:{passwordHash}});await tx.session.deleteMany({where:{userId:existing.id}});await tx.auditLog.create({data:{actorUserId:existing.id,action:"admin.password.initialized",resource:existing.id}});}
  else{if(admins)throw new Error("Admin bootstrap refused. Use existing account management.");const user=await tx.user.create({data:{email,passwordHash,name:"관리자",role:"SUPER_ADMIN",status:"ACTIVE"}});await tx.auditLog.create({data:{actorUserId:user.id,action:"admin.created",resource:user.id}});}
