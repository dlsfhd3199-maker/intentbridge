...
const passwordHash=process.env.DEV_SEED_PASSWORD?await hashPassword(process.env.DEV_SEED_PASSWORD):undefined;
for(const u of [{id:"dev-admin",email:"admin@intentbridge.test",name:"개발 관리자",role:"SUPER_ADMIN",advertiserId:null},{id:"dev-a",email:"a@intentbridge.test",name:"광고주 A 사용자",role:"ADVERTISER",advertiserId:"brand-a"},{id:"dev-b",email:"b@intentbridge.test",name:"광고주 B 사용자",role:"ADVERTISER",advertiserId:"brand-b"}]){await db.user.upsert({where:{email:u.email},update:passwordHash?{passwordHash}:{},create:{passwordHash,id:u.id,email:u.email,name:u.name,role:u.role,status:"ACTIVE"}});if(u.advertiserId)await db.advertiserMember.upsert({where:{userId_advertiserId:{userId:u.id,advertiserId:u.advertiserId}},update:{},create:{userId:u.id,advertiserId:u.advertiserId}});}
await db.$disconnect();
