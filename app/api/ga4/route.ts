...
async function handle(request:NextRequest,test=false){
 ...
 if(!test&&p.get("action")==="status")return send({mode,connection:user.role==="ADMIN"?connector.getConnectionStatus():{status:connector.getConnectionStatus().status,propertyId:null,lastTested:null,readOnly:true,quotaRemaining:null}});
 if(mode==="mock"&&!test)return send({mode,connection:user.role==="ADMIN"?connector.getConnectionStatus():{status:connector.getConnectionStatus().status,propertyId:null,lastTested:null,readOnly:true,quotaRemaining:null}});
 if(config.invalid||!config.propertyId||!config.keyFilename)return send({mode,connection:user.role==="ADMIN"?connector.getConnectionStatus():{status:connector.getConnectionStatus().status,propertyId:null,lastTested:null,readOnly:true,quotaRemaining:null},error:safeGA4Error(new GA4Error(config.invalid?"INVALID_PROPERTY":"MISSING_CONFIGURATION"))});
