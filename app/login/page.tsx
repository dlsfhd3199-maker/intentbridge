...
export default function Login(){
 const [mode,setMode]=useState<"login"|"signup"|"complete">("login"),[checking,setChecking]=useState(true),[checkError,setCheckError]=useState(false),[check,setCheck]=useState(0);
 ...
 lock.current=true;setBusy(true);try{
  if(mode==="signup"){
   ...
   const result=await signIn("credentials",{email,password,redirect:false,redirectTo:"/"});if(result?.error){setMessage(result.code==="pending"?"관리자 승인 대기 중입니다.":result.code==="disabled"?"사용이 중지된 계정입니다.":"이메일 또는 비밀번호를 확인해주세요.");return;}window.location.replace("/");
  }
