...
test("회원가입 → PENDING 차단 → 관리자 승인/연결 → ACTIVE 로그인 → Logout·세션 취소",async({page,browser})=>{
 ...
 try{
  await visitor.goto("http://localhost:3100/signup");await visitor.getByLabel("이름",{exact:true}).fill("가입 검수");await visitor.getByLabel("이메일",{exact:true}).fill(email);await visitor.getByLabel("비밀번호",{exact:true}).fill(process.env.TEST_LOGIN_PASSWORD!);await visitor.getByLabel("비밀번호 확인").fill(process.env.TEST_LOGIN_PASSWORD!);await visitor.getByRole("button",{name:"가입하기"}).click();await expect(visitor.getByRole("heading",{name:"가입이 완료되었습니다."})).toBeVisible();
