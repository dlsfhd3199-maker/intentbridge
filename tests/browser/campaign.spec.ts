...
test("Campaign 편집·Forecast·저장·상세·복원·광고주 분리", async ({page}) => {
  ...
  const create = page.getByRole("button",{name:"실행안 저장",exact:true});
  ...
  await expect(page.getByRole("combobox",{name:"실행안 목표",exact:true})).toHaveValue("Recover Cart");
  ...
  await page.getByRole("combobox",{name:"실행안 목표",exact:true}).selectOption("Lead"); await expect(create).toBeDisabled();
  await page.getByRole("combobox",{name:"실행안 목표",exact:true}).selectOption("Purchase");
  ...
  await page.getByLabel("계획 기간 (일)",{exact:true}).fill("0"); await expect(create).toBeDisabled();
  await page.getByLabel("계획 기간 (일)",{exact:true}).fill("7");
  await page.getByRole("combobox",{name:"검토 채널",exact:true}).selectOption("google"); await expect(create).toBeDisabled();
  await expect(page.getByTestId("campaign-audience")).toHaveText("0명");
  await page.getByRole("combobox",{name:"검토 채널",exact:true}).selectOption("meta");
  await page.getByRole("combobox",{name:"유입 경로",exact:true}).selectOption("naver"); await expect(create).toBeDisabled();
  await page.getByRole("combobox",{name:"유입 경로",exact:true}).selectOption("chatgpt");
  ...
  await page.getByRole("textbox",{name:"실행안 이름",exact:true}).fill("브랜드 A 캠페인");
  ...
  await expect(page.locator(".cs-list tbody")).toContainText("실행 준비");
  ...
  await saved(page);await page.reload(); await expect(page.getByRole("textbox",{name:"실행안 이름",exact:true})).toHaveValue("브랜드 A 캠페인");
  await page.getByLabel("광고주 워크스페이스").selectOption("brand-b"); await expect(page.locator(".cs-list tbody tr")).toHaveCount(0);
  await page.getByRole("textbox",{name:"실행안 이름",exact:true}).fill("브랜드 B 초안");
  ...
  await expect(page.getByRole("textbox",{name:"실행안 이름",exact:true})).toHaveValue("브랜드 B 초안");
  await page.getByLabel("광고주 워크스페이스").selectOption("brand-a");
  await expect(page.getByRole("textbox",{name:"실행안 이름",exact:true})).toHaveValue("브랜드 A 캠페인");
...
test("Funnel과 Performance 추천 및 저장된 성과 예측을 전달한다", async ({page}) => {
  ...
  await page.getByRole("button",{name:"대상 조건으로 실행안 작성",exact:true}).click();
  await expect(page.getByRole("combobox",{name:"세그먼트",exact:true})).toHaveValue("cart-14d");
  await expect(page.locator(".cs-origin")).toContainText("고객 여정");
  ...
  await page.getByRole("button",{name:"실행안 작성",exact:true}).first().click();
  await expect(page.locator(".cs-origin")).toContainText("성과 시뮬레이션");
  await expect(page.locator(".cs-origin")).toContainText("recommended");
  await expect(page.getByRole("button",{name:"실행안 저장",exact:true})).toBeEnabled();
  ...
  await saved(page);await page.reload(); await expect(page.locator(".cs-origin")).toContainText("성과 시뮬레이션");
});
