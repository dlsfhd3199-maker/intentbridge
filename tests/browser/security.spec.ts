...
test("관리자 UI에서 광고주 생성·수정·사용자 승인를 저장하고 Audit에 기록한다",async({page})=>{
 ...
 try{
  await page.goto("/advertisers");await page.getByLabel("광고주 이름",{exact:true}).fill(name);await page.getByLabel("업종",{exact:true}).fill("검수");await page.getByLabel("대표 상품",{exact:true}).fill("검수 상품");await page.getByRole("button",{name:"광고주 생성",exact:true}).click();await expect(page.getByRole("region",{name:"광고주 성과",exact:true}).locator("tbody tr")).toContainText(["브랜드 A","브랜드 B",name]);
