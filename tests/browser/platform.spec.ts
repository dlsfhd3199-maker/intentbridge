...
test('새 Workspace: 빈 상태 → 프로필 → 실제 담당자 → 데모 연결 → 검증 → 운영 시작',async({page})=>{
...
test('새 플랫폼 API의 MANAGER 조회/쓰기 제한·ADVERTISER 격리',async({page})=>{
 ...
 try{
  ...
  await expect(page.locator('a[href^="/connections"]')).toHaveCount(0);await expect(page.getByLabel('광고주 · 캠페인 검색')).toHaveCount(0);
 }finally{await db.user.deleteMany({where:{id:manager.id}});}
});
test('검색·최근 광고주·알림 읽음·보고서 기간·회사 표시 저장',async({page})=>{
 await page.getByLabel('광고주 · 캠페인 검색',{exact:true}).fill('브랜드 B');await page.locator('.platform-search-results').getByRole('link',{name:'브랜드 B',exact:true}).click();await expect(page.getByLabel('광고주 워크스페이스')).toHaveValue('brand-b');
