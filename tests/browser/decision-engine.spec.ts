...
test("Decision: Signal 근거→Journey 강조→Audience→Campaign 원본 보존",async({page})=>{
 ...
 await drop.getByRole("link",{name:"고객 여정 확인"}).click();await expect(page).toHaveURL(/signalStage=BeginCheckout/);await expect(page.locator('.fw-stage').filter({hasText:"BeginCheckout"})).toHaveAttribute("aria-pressed","true");
 await page.goto("/operations?advertiser=brand-a&period=30");await feed.locator(`[data-signal-id="${id}"]`).getByRole("button",{name:"대상 조건으로 실행안 작성"}).click();
 ...
 await dialog.getByRole("button",{name:"실행안 작성",exact:true}).click();await expect(page.locator(".cs-origin")).toContainText("Decision Trail");await expect(page.locator(".cs-origin")).toContainText("장바구니 이후 이탈");await expect(page.locator("#cs-audience")).toContainText(size);await expect(page.locator("#cs-review")).toContainText(`구매 ${purchase}`);
 await expect(page.getByRole("combobox",{name:/검토 채널/})).toHaveValue("meta");await page.getByRole("combobox",{name:/검토 채널/}).selectOption("naver");await expect(page.locator("#cs-channel")).toContainText("초안 저장만 가능합니다");await expect(page.getByRole("button",{name:"실행안 저장",exact:true})).toBeDisabled();await page.getByRole("combobox",{name:/검토 채널/}).selectOption("meta");
...
test("Decision: ADVERTISER 390px 조회 전용·ID 조작403·외부 호출 없음",async({page})=>{
 ...
 await page.setViewportSize({width:390,height:844});await loginAs(page,"a@intentbridge.test");const feed=page.getByTestId("decision-feed");await expect(feed.locator('.decision-row').first()).toBeVisible();await expect(feed).not.toContainText("브랜드 B");await expect(feed.getByRole("button",{name:"대상 조건으로 실행안 작성"})).toHaveCount(0);await expect(feed.getByRole("combobox")).toHaveCount(0);await feed.locator('summary').first().click();
...
test("Decision: 연결 오류가 있으면 DATA_ISSUE만 노출하고 회수 설계 억제",async({page})=>{
 ...
 await page.goto('/operations?advertiser=brand-a&period=30');const feed=page.getByTestId('decision-feed');await expect(feed.locator('.decision-row')).toHaveCount(1);await expect(feed.locator('.decision-row')).toHaveAttribute('data-signal-type','DATA_ISSUE');await expect(feed.getByRole('button',{name:'대상 조건으로 실행안 작성'})).toHaveCount(0);await expect(feed).toContainText('긴급 확인');
});
