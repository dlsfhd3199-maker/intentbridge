...
async function createCampaign(page:Page,name="운영 Cart") {
  ...
  await page.getByRole("textbox",{name:"실행안 이름",exact:true}).fill(name);
  await page.getByLabel("일 예산 (원)",{exact:true}).fill("10000");
  await page.getByRole("button",{name:"실행안 저장",exact:true}).click();
  ...
  await page.getByRole("link",{name:/운영 제안 Review/}).click();
  await expect(page.getByRole("heading",{name:"실행안 상태와 예시 관측값",exact:true})).toBeVisible();
}
test("Rule AND·Guardrail·실행안에 반영·Rollback·History·Version·Alert 복원",async({page})=>{
  ...
  await card.getByRole("button",{name:"실행안에 반영",exact:true}).click();
  await expect(page.locator(".op-notice")).toContainText("실행안 반영 완료");
  await page.getByRole("combobox",{name:"History Filter",exact:true}).selectOption("Budget");
  const history=page.locator("section").filter({has:page.getByRole("heading",{name:"실행안 변경 기록",exact:true})});
  await expect(history.locator("tbody")).toContainText("₩10,000 → ₩11,000");
  await history.getByRole("button",{name:"이전 실행안 복원",exact:true}).first().click();
  await expect(page.locator(".op-notice")).toContainText("이전 실행안을 복원");
  ...
  await page.getByRole("button",{name:"목표·안전 제한 저장",exact:true}).click();
  await expect(page.locator(".op-queue article").filter({hasText:"Cart 증액 Rule"}).first().getByRole("button",{name:"실행안에 반영",exact:true})).toBeDisabled();
...
test("여러 Draft 열기·복제·삭제, Campaign 비교와 JSON/CSV/HTML 다운로드",async({page})=>{
  await page.goto("/campaigns");
  await page.getByRole("textbox",{name:"실행안 이름",exact:true}).fill("Checkout 초안");
  await page.getByRole("button",{name:"초안 저장",exact:true}).click();
  await page.getByRole("button",{name:"새 초안",exact:true}).click();
  await page.getByRole("textbox",{name:"실행안 이름",exact:true}).fill("Cart 초안");
  await page.getByRole("combobox",{name:"세그먼트",exact:true}).selectOption("cart-14d");
  await page.getByRole("button",{name:"초안 저장",exact:true}).click();
  const library=page.locator("section").filter({has:page.getByRole("heading",{name:"실행안 초안",exact:true})});
  ...
  await expect(page.getByRole("textbox",{name:"실행안 이름",exact:true})).toHaveValue("Checkout 초안");
  await saved(page);await page.reload();await expect(library.locator("tbody tr")).toHaveCount(2);
  await page.getByRole("link",{name:/운영 제안 Review/}).click();
  await page.getByRole("combobox",{name:"비교 실행안 1",exact:true}).selectOption({index:1});
  await page.getByRole("combobox",{name:"비교 실행안 2",exact:true}).selectOption({index:2});
  await expect(page.locator('[aria-label="실행안 비교"]')).toContainText("Forecast Purchase");
  for(const format of ["JSON","CSV","HTML"]){
    ...
    if(format==="CSV")expect(text).toContain('"운영 제안 규칙"');
    if(format==="HTML"){expect(text).toContain("실행안");const report=await page.context().newPage();await report.setContent(text);await expect(report.getByRole("heading",{name:"실행안",exact:true})).toBeVisible();await report.screenshot({path:"test-results/campaign-plan-report.png",fullPage:true});await report.close();}
  }
});
test("Named Simulation 저장·비교·복제·삭제·Performance 재열기·Campaign 전환",async({page})=>{
  ...
  await page.getByRole("link",{name:/운영 제안 Review/}).click();
  await library.locator("tbody tr").filter({hasText:"Recommended Plan"}).first().getByRole("button",{name:"실행안 작성",exact:true}).click();
  await expect(page.locator(".cs-origin")).toContainText("recommended");
});
test("Operations의 데스크톱/모바일 레이아웃과 Home 운영 요약",async({page})=>{
  await createCampaign(page);
  for(const width of [1440,390,320]){await page.setViewportSize({width,height:1000});await expect(page.getByRole("heading",{name:"오늘의 운영 제안",exact:true})).toBeVisible();expect(await page.evaluate(()=>document.documentElement.scrollWidth>innerWidth),`${width}px`).toBe(false);await page.screenshot({path:`test-results/operations-${width}.png`,fullPage:true});}
  await page.setViewportSize({width:1440,height:1000});
  await page.getByRole("link",{name:/대시보드 Overview/}).click();await page.getByText("선택한 광고주 상세 성과 보기",{exact:true}).click();await expect(page.locator(".operations-home")).toContainText("검토 완료 실행안1");
  await page.getByRole("link",{name:"운영 제안 열기 →",exact:true}).click();await expect(page.getByRole("heading",{name:"오늘의 운영 제안",exact:true})).toBeVisible();
});
