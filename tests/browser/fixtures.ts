...
export async function loginAs(page:Page,email="admin@intentbridge.test"){
 ...
 const response=await page.request.post("/api/auth/signin/resend",{form:{email,csrfToken:csrf.csrfToken,callbackUrl:"http://localhost:3100/"},headers:{"X-Auth-Return-Redirect":"1"},maxRedirects:0});expect(response.status()).toBeLessThan(400);
 const mail=await (await page.request.get(`http://127.0.0.1:3101/mail?email=${encodeURIComponent(email)}`)).json();expect(mail?.text).toBeTruthy();const url=mail.text.match(/http:\/\/localhost:3100\/api\/auth\/callback\/resend\?[^\s]+/)?.[0];expect(url).toBeTruthy();await page.goto(url);await expect(page.locator("nav")).toBeVisible();sessions.set(email,await page.context().cookies());
}
