...
export async function loginAs(page:Page,email="admin@intentbridge.test"){
 ...
 if(cached){await page.context().addCookies(cached);if((await page.request.get("/api/bootstrap")).ok()){await page.goto("/dashboard");return;}sessions.delete(email);await page.context().clearCookies();}
 await page.goto("/login");const csrf=await (await page.request.get("/api/auth/csrf")).json();
 const response=await page.request.post("/api/auth/callback/credentials",{form:{email,password:process.env.TEST_LOGIN_PASSWORD!,csrfToken:csrf.csrfToken,callbackUrl:"http://localhost:3100/dashboard"},headers:{Origin:"http://localhost:3100","X-Auth-Return-Redirect":"1"},maxRedirects:0});expect(response.status()).toBeLessThan(400);expect((await response.json()).url).not.toContain("error=");
 await page.goto("/dashboard");await expect(page.locator("nav")).toBeVisible();sessions.set(email,await page.context().cookies());
}
