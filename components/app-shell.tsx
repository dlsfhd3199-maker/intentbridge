  ...
  return <SelectionContext.Provider value={(id, days) => { if (canAccessAdvertiser(user,id)&&advertisers.some(a => a.id === id)) { selectWorkspace(id,days); } }}><DashboardContext.Provider value={data}>
    ...
    <div className="app-shell" data-density={density}>
      <aside className="sidebar">
        <Link href="/dashboard" className="brand" aria-label="IntentBridge 홈"><span className="brand-symbol"/><span>IntentBridge<small>고객 흐름 워크스페이스</small></span></Link>
        <div className="workspace-label">WORKSPACE <span>{user.role==="manager"?"담당 운영":user.role==="advertiser"?"성과 조회":"전체 운영"}</span></div>
        <nav aria-label="주 메뉴">{items.map(({ href, name, caption }) => {const Icon=navIcons[href]??LayoutDashboard;return <Link key={href} aria-label={`${name} ${caption}`} href={["/campaigns","/operations"].includes(href) ? `${href}?advertiser=${scopeId}&period=${period}` : href} className={`nav-item ${pathname === href ? "active" : ""}`} aria-current={pathname === href ? "page" : undefined}><Icon size={18} strokeWidth={1.6}/><span>{name}</span>{pathname === href && <ChevronRight size={14}/>}</Link>})}{user.role==="manager"&&<Link href="/dashboard#assigned-workspaces" className="nav-item"><Building2 size={18} strokeWidth={1.6}/><span>내 광고주</span></Link>}</nav>
      ...
      <div className="main-shell">
        <main id="main" className="main-content">
          <header className={`context-header ${pathname==="/dashboard"?"ds-dashboard-utility":""}`}><div><h1>{current?.name??"접근 안내"}</h1><p className="ui-page-description">{pageDescriptions[pathname]}</p></div>{pathname!=="/advertisers"&&<div className="advertiser-control"><label htmlFor="advertiser">광고주 워크스페이스</label>{selectable?<select id="advertiser" value={scopeId} onChange={event=>selectWorkspace(event.target.value,period)}>{advertisers.map(item=><option key={item.id} value={item.id}>{item.name} · {item.productName}</option>)}</select>:<strong className="workspace-lock">{advertisers.find(a=>a.id===scopeId)?.name} · 내 워크스페이스 <LockKeyhole size={14}/></strong>}</div>}<div className="period-control"><span>조회 기간</span><div className="period-buttons" role="group" aria-label="조회 기간">{([7,14,30] as const).map(days=><button key={days} aria-pressed={period===days} onClick={()=>selectWorkspace(scopeId,days)} className={period===days?"selected":""}>{days}일</button>)}</div></div></header>
          {pathname!=="/advertisers"&&<><PlatformToolbar advertisers={advertisers} id={scopeId} period={period} select={selectWorkspace}/><div className="brand-context"><b>{advertisers.find(a=>a.id===scopeId)?.name}</b><span>{advertisers.find(a=>a.id===scopeId)?.productName}</span></div></>}
