...
const pageDescriptions:Record<string,string>={"/":"유입부터 구매까지, 놓친 고객과 다음 운영 액션을 연결합니다.","/advertisers":"광고주별 성과와 워크스페이스를 관리합니다.","/funnel":"고객을 놓치는 구간과 다시 만날 기회를 확인합니다.","/performance":"현재 데이터를 기준으로 개선안과 예상 변화를 비교합니다.","/campaigns":"고객 그룹과 메시지를 설계하고 캠페인 상태를 확인합니다.","/operations":"검토가 필요한 운영 제안을 확인하고 다음 액션을 결정합니다.","/reports":"이번 기간 성과와 개선 기회를 한 장의 보고서로 읽습니다.","/connections":"데이터 출처와 채널별 연결 상태를 확인합니다.","/settings":"사용자, 담당 광고주와 시스템 설정을 관리합니다."};
const navIcons:Record<string,typeof LayoutDashboard>={"/":LayoutDashboard,"/advertisers":Building2,"/funnel":Route,"/performance":SlidersHorizontal,"/campaigns":Megaphone,"/operations":ListChecks,"/reports":FileChartColumn,"/connections":Cable,"/settings":Settings2};
  ...
  return <SelectionContext.Provider value={(id, days) => { if (canAccessAdvertiser(user,id)&&advertisers.some(a => a.id === id)) { selectWorkspace(id,days); } }}><DashboardContext.Provider value={data}>
    ...
    <div className="app-shell" data-density={density}>
      <aside className="sidebar">
        <Link href="/" className="brand" aria-label="IntentBridge 홈"><span className="brand-symbol"/><span>IntentBridge<small>ADVERTISING WORKSPACE</small></span></Link>
        <div className="workspace-label">WORKSPACE <span>{user.role==="manager"?"담당 운영":user.role==="advertiser"?"성과 조회":"전체 운영"}</span></div>
        <nav aria-label="주 메뉴">{items.map(({ href, name, caption }) => {const Icon=navIcons[href]??LayoutDashboard;return <Link key={href} aria-label={`${name} ${caption}`} href={["/campaigns","/operations"].includes(href) ? `${href}?advertiser=${scopeId}&period=${period}` : href} className={`nav-item ${pathname === href ? "active" : ""}`} aria-current={pathname === href ? "page" : undefined}><Icon size={18} strokeWidth={1.6}/><span>{name}<small>{caption}</small></span>{pathname === href && <ChevronRight size={14}/>}</Link>})}{user.role==="manager"&&<Link href="/#assigned-workspaces" className="nav-item"><Building2 size={18} strokeWidth={1.6}/><span>내 광고주</span></Link>}</nav>
