...
COPY --from=builder --chown=node:node /app/lib/auth-diagnostics.ts /app/lib/password.ts ./lib/
...
COPY --from=builder --chown=node:node /app/scripts/check-env.ts /app/scripts/create-admin.ts /app/scripts/diagnose-auth.ts /app/scripts/initialize-password.ts ./scripts/
USER node
EXPOSE 3000
