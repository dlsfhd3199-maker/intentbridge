...
export async function requireUser(){const session=await auth();if(!session?.user?.id)throw new AccessError(401,"로그인이 필요합니다.");const user=await db.user.findUnique({where:{id:session.user.id},include:{members:{where:{advertiser:{status:"ACTIVE"}},select:{advertiserId:true}}}});if(!user||user.status!=="ACTIVE")throw new AccessError(401,"로그인이 필요합니다.");return user;}
export function sessionUser(user:Awaited<ReturnType<typeof requireUser>>):SessionUser {const ids=user.members.map(m=>m.advertiserId);return {id:user.id,email:user.email,name:user.name??"",role:user.role==="ADMIN"?"admin":"advertiser",advertiserId:ids[0]??"",advertiserIds:ids,source:"session"};}
...
export async function requireAdvertiserAccess(advertiserId:string,permission?:Permission){const user=await requireUser();if(permission&&!can(sessionUser(user).role,permission))throw new AccessError(403);const advertiser=await db.advertiser.findFirst({where:{id:advertiserId,status:"ACTIVE",...(user.role==="ADMIN"?{}:{members:{some:{userId:user.id}}})}});if(!advertiser)throw new AccessError(403);return {user,advertiser};}
