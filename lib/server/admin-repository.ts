...
const safeUser={id:true,email:true,name:true,role:true,status:true,lastLoginAt:true,members:{select:{advertiserId:true}}} as const;
const text=(v:unknown,max=120)=>v===undefined?"":boundedString(v,max);
export async function listUsers(url?:string){await requirePermission("MANAGE_SETTINGS");return db.user.findMany({...pagination(url),select:safeUser,orderBy:{id:"asc"}});}
export async function changeUser(input:Record<string,unknown>,invite=false){
 ...
 return transaction(async tx=>{
  ...
  await tx.auditLog.create({data:{requestId:requestContext()?.requestId,actorUserId:actor.id,action:invite?"USER_INVITED":"USER_UPDATED",resource:user.id,...(prior?{before:JSON.parse(JSON.stringify(prior))}:{}),after:JSON.parse(JSON.stringify(after))}});return after;
