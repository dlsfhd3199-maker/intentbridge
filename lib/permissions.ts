...
const managerPermissions:Permission[]=[...advertiserPermissions,"MANAGE_CAMPAIGN","MANAGE_OPERATIONS","VIEW_OPERATIONS","VIEW_CONNECTIONS","VIEW_WORKSPACE_SUMMARIES"];
export function can(role:UserRole,permission:Permission){return role==="super_admin"||role==="manager"&&managerPermissions.includes(permission)||role==="advertiser"&&advertiserPermissions.includes(permission);}
export function roleFromDatabase(role:string):UserRole|undefined{return role==="SUPER_ADMIN"?"super_admin":role==="MANAGER"?"manager":role==="ADVERTISER"?"advertiser":undefined;}
...
export function currentUser():SessionUser {if(runtimeScope())return runtimeScope()!.user;if(resolvedUser)return resolvedUser;if(typeof window==="undefined")return mockUser("super_admin");throw new Error("로그인이 필요합니다.");}
