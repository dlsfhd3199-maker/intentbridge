...
export const defaultAppSettings:AppSettings={companyName:"IntentBridge",logoText:"IB",timezone:"Asia/Seoul",defaultPeriod:30,currency:"KRW",locale:"ko-KR",targetRoas:400,targetCpa:25000,forecastScenario:"recommended",density:"comfortable"};
export function validAppSettings(v:unknown):v is AppSettings {return isRecord(v)&&(v.companyName===undefined||(typeof v.companyName==="string"&&v.companyName.trim().length>0&&v.companyName.length<=60))&&(v.logoText===undefined||(typeof v.logoText==="string"&&v.logoText.trim().length>0&&v.logoText.length<=4))&&(v.timezone===undefined||v.timezone==="Asia/Seoul")&&[7,14,30].includes(v.defaultPeriod as number)&&v.currency==="KRW"&&v.locale==="ko-KR"&&nonnegative(v.targetRoas)&&v.targetRoas>0&&v.targetRoas<=1e6&&nonnegative(v.targetCpa)&&v.targetCpa>0&&v.targetCpa<=1e9&&["conservative","recommended","aggressive"].includes(String(v.forecastScenario))&&["compact","comfortable"].includes(String(v.density));}
